# Platforms for a US Systematic Trader: Research Notes, August 2026

**State coverage: availability is set per state and moves. The current map is tracked publicly** ([CBS Sports state tracker](https://www.cbssports.com/prediction/news/prediction-market-legal-states/)).
...
- **Legality:** CFTC DCM. ~49 states + DC (NV blocked; sports restricted in a few states). 18+, subject to the exchange's own verification.
...
- CFTC DCM; intermediated (FCM) model; **USD** (ACH/debit/Apple Pay, $50k/day; wire min $1k); no crypto wallet needed. Onboarding: photo ID plus a selfie check.
